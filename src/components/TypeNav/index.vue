...
export default {
  ...
  methods: {
    ...
    // },
    //throttle回调函数别用剪头函数，可能出现上下文this
    ...
    goSearch(event) {
      //最好的解决方案：编程式导航+事件委派
      //利用事件委派存在一些问题：1.点击一定是a标签2：如何获取参数
      //存在的一些问题：事件委派，是把全部的子节点【h3,dt,dl,em】的事件委派给父亲节点
      //点击a标签的时候，才会进行路由跳转【怎么能确定点击的一定是a标签】
      //存在的另一问题：即使你能确定点击的是a标签，如何区分是一级、二级、三级分类的标签
      //利用自定义属性去解决
      //第一个问题：把子节点当中a标签，加上自定义属性data-categoryNmae,其余的子节点是没有的
